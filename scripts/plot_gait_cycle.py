#!/usr/bin/env python3
"""Right hip + knee angles for one gait cycle — centred on peak knee flexion.
...
OUT = '/home/elec330-admin/ros2_ws/src/pady_robot/prerequisite+docs/figures/gait_cycle_joint_angles.png'
...
# Original cycle: t=1.279–3.253 (1.974s), knee peak at t=2.113
# Re-centre: start = 2.113 − 0.987 = 1.126, end = 2.113 + 0.987 = 3.100
c_start = np.argmin(np.abs(t - 1.126))
c_end   = np.argmin(np.abs(t - 3.100))
tc  = t[c_start:c_end+1]

# URDF hip offset correction
HIP_RIGHT_OFFSET = 0.28  # rad (from URDF joint origin rpy)
hip_plot  = hr[c_start:c_end+1] + np.degrees(HIP_RIGHT_OFFSET)
knee_plot = -kr[c_start:c_end+1]

pct = (tc - tc[0]) / (tc[-1] - tc[0]) * 100

# ── PaDy phase boundaries (from corrected hip kinematics, re-centred) ──
# 0-9%:   Swing (hip flexing toward peak, knee locked)
# 9-15%:  DS (heel strike region)
# 15-58%: Stance (knee loading response, peak −31° at ~49%)
# 58-65%: DS (pre-swing)
# 65-100%: Swing (hip extending, knee locked)
pady_phases = [
    (0,    9,   'Swing',   '#ffcc80'),
    (9,   15,   'DS',      '#b0bec5'),
    (15,  58,   'Stance',  '#a5d6a7'),
...
ax.plot(pct, hip_plot, '-', color='#2196F3', lw=2.2, label='PaDy Hip (right)')
ax.plot(pct, knee_plot, '-', color='#e74c3c', lw=2.2, label='PaDy Knee (right)')
...
fig.suptitle('Comparison of Mean Measured Joint Angles — Right Hip, Knee',
